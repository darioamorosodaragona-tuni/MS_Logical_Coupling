...
def introducing(projects_to_compare, first_last_commit, plot_ax=None):
    for project in tqdm(projects_to_compare):
        ...
        date_range_data = first_last_commit[
           first_last_commit["project"] == project]
        ...
        for group_name, df_group in group_date_range:
            if df_group.empty:
                ...
                result['n_commits'][last_index] = 0
            else:
                result['n_commits'][df_group['group_index'].iloc[0]] = len(df_group['commit_hash'].unique().tolist())
        ...
        r.plot(y="n_commits", ax=ax, style="-")
        r.plot(y="n_commits", ax=ax, style="*")
...
if __name__ == "__main__":
    ...
    introducing(projects, first_last_commit=date_range_data)
